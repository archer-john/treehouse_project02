...
class Affine(Cipher):
    """
    Affine cipher is a type of monoalphabetic substitution cipher, wherein each
    letter in an alphabet is mapped to its numeric equivalent.
    ...
    def get_alpha(self):
        """
        Request user to enter alpha key. The input is validated as an int and
        that it is a coprime of m (self.m)
        :return: int
        """
        m_coprimes = [num for num in range(self.m) if self.coprime(num, self.m)]
        user_input = input(f"Please pick a number from the list to be used "
                           f"as the first\n key: {m_coprimes} ")
        while True:
            if user_input.isdigit():
                if int(user_input) in m_coprimes:
                    ...
                    print(f"[!] - You must use a number from the list as your "
                          f"first key. list: {m_coprimes}")
            else:
    ...
    def get_beta(self):
        """
        Request user to enter beta key. The input is validated as a
        non-negative int
        :return: int
        """
        user_input = input("Please enter a non-negative integer as your "
                           "second key: ")
        while True:
            if user_input.isdigit() and int(user_input) > 0:
                return int(user_input)
            else:
                user_input = input("[!] - Incorrect value entered. Please pick "
                                   "again: ")
    ...
    def decrypt(self, text: str):
        """
        Iterates over an encrypted string applying the Affine Decryption
        function.
        :param text: user entered string
